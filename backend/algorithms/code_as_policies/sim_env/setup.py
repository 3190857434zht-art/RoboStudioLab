...
import numpy as np
import os
from config.constants import BOUNDS,COLORS,CORNER_POS,PIXEL_SIZE
from Robotics.arm import Robotiq2F85
# Gym-style environment code

class PickPlaceEnv():

  def __init__(self, asset_path, render=False, high_res=False, high_frame_rate=False):
    ...
    algorithm_root_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
  ...
  def reset(self, object_list):
    ...
    self.robot_id = pybullet.loadURDF("/algorithm/code_as_policies/assets/ur5e/ur5e.urdf", [0, 0, 0], flags=pybullet.URDF_USE_MATERIAL_COLORS_FROM_MTL)
    self.ghost_id = pybullet.loadURDF("/algorithm/code_as_policies/assets/ur5e/ur5e.urdf", [0, 0, -10])
    ...
    self.gripper = Robotiq2F85(self.robot_id, self.ee_link_id, self.asset_path)
    ...
    for obj_name in object_list:
      if ('block' in obj_name) or ('bowl' in obj_name):
        ...
        if object_type == 'block':
          ...
          object_position[2] = 0
          object_id = pybullet.loadURDF("/algorithm/code_as_policies/assets/bowl/bowl.urdf", object_position, useFixedBase=1)
  ...
  _ZH_COLOR_MAP = {
    ...
    '灰色': 'gray', '白色': 'white', '青色': 'cyan', '黑色': 'black',
  }
  ...
  def get_obj_id(self, obj_name):
    ...
    if obj_id is None:
      raise ValueError(f'Object not found: "{obj_name}". Available: {list(self.obj_name_to_id.keys())}')
    return obj_id
